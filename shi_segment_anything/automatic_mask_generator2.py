...
#         self,
#         model: Sam,
#         points_per_side: Optional[int] = 40,
...
#     @torch.no_grad()
#     def generate(self, image: np.ndarray,ref_prompt,SLIC_centers, super_pixel) -> List[Dict[str, Any]]:
#         """
...
#         # Generate masks
#         mask_data = self._generate_masks(image,ref_prompt,SLIC_centers, super_pixel)
...
#     def _generate_masks(self, image: np.ndarray,nop,SLIC_centers, super_pixel) -> MaskData:
...
#  #       sim_map2 = resize(sim,orig_size,preserve_range=True)
#   #      print(sim_map2)
#         # sim_map2 = cv2.convertScaleAbs(sim_map2*255)
#         # print(sim_map2)
#    #     numer = sim_map2 - np.min(sim_map2)
#     #    denom = (sim_map2.max() - sim_map2.min()) +0.01
#      #   sim_map2 = numer/ denom
#       #  sim_map2 = (sim_map2 * 255).astype("uint8")
#        # print(sim_map2)
#         # colormap=cv2.COLORMAP_PLASMA
#         # colormap=cv2.COLORMAP_VIRIDIS
#        # heatmap = cv2.applyColorMap(sim_map2, cv2.COLORMAP_JET)
#         # th,sim_map2 = cv2.threshold(sim_map2,0,1,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
...
#         #output = cv2.addWeighted(img_bgr, 0.5, heatmap, 1 - 0.5, 0)
...
#         # # self.point_grids = build_point_grid(int(number_of_points))
...
#         else:
#           # points_per_side = np
#           self.point_grids = build_point_grid(int(nop))
#           points_scale = np.array(orig_size)[None, ::-1]
#           points_for_image = self.point_grids * points_scale
...
class SamAutomaticMaskGenerator:
    def __init__(
        ...
        # self.prototype_color = defaultdict(list)

    @torch.no_grad()
    def generate(self, image: np.ndarray, ref_bbox,SLIC_centers,super_pixel, pn) -> List[Dict[str, Any]]:
        """
        Generates masks for the given image.
        ...
        # Generate masks
        # mask_data = neg,mask_data2 = pos,mask_data3 = all
        self.point_grids = build_all_layer_point_grids(
                pn,
                self.crop_n_layers,
                self.crop_n_points_downscale_factor,
        ...
        mask_data, mask_data2 = self._generate_masks(image, ref_bbox,SLIC_centers,super_pixel, pn)
        ...
        for idx in range(len(mask_data2["segmentations"])):
            ann = {
                ...
                # "crop_box": box_xyxy_to_xywh(mask_data2["crop_boxes"][idx]).tolist(),
                "label":0
    ...
    def _generate_masks(self, image: np.ndarray, ref_box,SLIC_centers,super_pixel, pn) -> MaskData:
        orig_size = image.shape[:2]
        ...
        self.predictor.set_image(image)  
        # ref_points = []
        # ref_box = pre_process_ref_box(ref_box, crop_box, 0)
        ...
        # for b in ref_box:
        #     ref_points.append([(b[0]+b[2])//2,(b[1]+b[3])//2])
        # ref_points = np.array(ref_points)
        # ref_points = torch.tensor(ref_points, device=self.predictor.device)
        # transformed_points = self.predictor.transform.apply_coords_torch(ref_points, orig_size)
        # in_labels = torch.ones(transformed_points.shape[0], dtype=torch.int, device=self.predictor.device)
...
# text!!!!!!!!!!!!!!!!!!!!!
        ...
        # area = torch.sum(masks, dim = (2,3),keepdim=True)
        # # print(area)
        # print(area.min())
        # if area.min()<150:
...
# text!!!!!!!!!!!!!!!!!!!!!
        ...
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            # print(layer_idx)
            crop_data = self._process_crop(image, crop_box, layer_idx, orig_size, ref_box,SLIC_centers,super_pixel, pn)
