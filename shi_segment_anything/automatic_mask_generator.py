  ...
  #      print(sim_map2)
   ...
   #     numer = sim_map2 - np.min(sim_map2)
      ...
      #  sim_map2 = (sim_map2 * 255).astype("uint8")
       ...
       # cv2.imwrite(str(time.time())+".jpg", output)
        ...
        for (points,) in batch_iterator(self.points_per_batch, points_for_image):
            ...
            if len(points_new)>0:
                batch_data, point_mask = self._process_batch(np.array(points_new), orig_size, sim_map, target_embedding, point_mask, ref_prompt)
    ...
    def _process_batch(
        ...
        # newcord = [transformed_points[:,1],transformed_points[:,0]]
        # print(newcord)
        # # print(transformed_points[:,1])

        # print(sim_map.shape)
        # exit()
        # x0,y0,x1,y1 = ref_prompt
        ...
        # print(masks.shape)
        # exit()
        # masks2, iou_preds2, logits2 = self.predictor.predict_torch(
        ...
        # data2 = MaskData(
        #     masks=masks2.flatten(0, 1),
        #     iou_preds=iou_preds2.flatten(0, 1),
        #     points=torch.as_tensor(points.repeat(masks2.shape[1], axis=0)),
        # )
        # del masks2
        ...
        # print(data["stability_score"].shape)
